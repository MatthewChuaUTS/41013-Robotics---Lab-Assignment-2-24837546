...
for waypoint = 1:size(niryoTrajectoryQmatrix, 2)
    % for jointStep = 1:size(niryoTrajectoryQmatrix{1}, 1)
    myNiryoOne.model.animate(niryoTrajectoryQmatrix(waypoint, :));
    myUR3.model.animate(ur3TrajectoryQmatrix(waypoint, :));
    niryoEndEffectorPose = myNiryoOne.model.fkine(niryoTrajectoryQmatrix(waypoint, :)).T;
    ur3EndEffectorPose = myUR3.model.fkine(ur3TrajectoryQmatrix(waypoint, :)).T;
    ...
    %     sponge.PlotSponge(spongePose);
    % 
    % elseif (waypoint >= 500 && waypoint <= 700)
    %     % plateAttached = true;
    %     platePose = ur3EndEffectorPose * transl(0, 0, 0.025) * trotx(pi/2);
    %     plate.PlotPlate(platePose);   
    %     % spongeAttached = true;
