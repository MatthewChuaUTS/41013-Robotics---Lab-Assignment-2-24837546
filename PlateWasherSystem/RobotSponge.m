classdef RobotSponge < handle
    ...
    methods
        %% Constructor
        function self = RobotSponge(baseTr)
            self.spongeModel = self.getSpongeModel('plate');
            self.spongeModel.base = baseTr;
            % Plot 3D model
            % plot3d(self.piggyModel, 0, 'base', false);
            % self.PlotAndColourRobot();
            plot3d(self.spongeModel, 0, 'workspace', [-1.6, 0.6, -1.6, 0.6, -0.7971, 1], 'view', [-30,30], 'noarrow', 'nowrist', 'color', {'hotpink'}, 'notiles');
            hold on;
        end
       
        %% Move the piggy forward and rotate randomly
        function moveSponge(self, position)
            % Update piggy position
            self.spongeModel.base = position;
            animate(self.spongeModel, 0);                
            drawnow();
        end    
    end
    ...
    methods (Static)
        ...
        function model = getSpongeModel(name)
            ...
            [faceData, vertexData] = plyread('sponge.ply', 'tri');
