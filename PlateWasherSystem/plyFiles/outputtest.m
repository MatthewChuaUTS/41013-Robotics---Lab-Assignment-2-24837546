...
while (isCollision)
    ...
    for i = startWaypoint:size(qWaypoints,1)-1                              % from startwaypoint to last one
        ...
        if ~IsCollision(myNiryoOne,qMatrixJoin,faces,vertex,faceNormals)
            ...
            qMatrixJoin = InterpolateWaypointRadians([qMatrix(end,:); qWaypointsJoints(end, :);],deg2rad(10));
            if ~IsCollision(myNiryoOne,qMatrixJoin,faces,vertex,faceNormals)
...
% and triangle obstacles in the environment (faces,vertex,faceNormals)
    function result = IsCollision(myNiryoOne,qMatrix,faces,vertex,faceNormals,returnOnceFound)
if nargin < 6
...
for qIndex = 1:size(qMatrix,1)
    ...
    tr = GetLinkPoses(qMatrix(qIndex,:), myNiryoOne);
...
while ~isempty(find(maxStepRadians < abs(diff(jtraj(q1,q2,steps))),1))
    steps = steps + 1; % divide steps into smaller steps, add into qmatrix
end
